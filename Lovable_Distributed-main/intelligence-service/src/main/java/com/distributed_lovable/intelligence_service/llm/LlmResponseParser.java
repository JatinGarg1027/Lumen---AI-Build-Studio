...
public class LlmResponseParser {
    ...
    // This below is how my llm will generate message
    /**
     * Regex Breakdown:
    ...
    private static final Pattern GENERIC_TAG_PATTERN = Pattern.compile( // to fetch the tags (message tag,file tag,tool tag)
            "(<(message|file|tool)([^>]*)>)([\\s\\S]*?)(</\\2>)",
            Pattern.CASE_INSENSITIVE | Pattern.DOTALL
    );

    // Helper to extract specific attributes (path="..." or args="...") from Group 3
    private static final Pattern ATTRIBUTE_PATTERN = Pattern.compile( // to fetch the path and argument from response
            "(path|args)=\"([^\"]+)\""
    );

    public List<ChatEvent> parseChatEvents(String fullResponse, ChatMessages parentMessage)
    {
