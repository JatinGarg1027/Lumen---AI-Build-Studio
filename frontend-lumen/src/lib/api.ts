...
const BASE_URL = import.meta.env.VITE_API_URL || "/api";  // for backend
...
export const api = {
  async login(credentials: LoginCredentials): Promise<LoginResponse> {
    const response = await fetch(`${BASE_URL}/v1/account/auth/login`, {
      method: "POST",
    ...
    if (data && data.userProfileResponse && !data.user) {
      data.user = data.userProfileResponse;
    }
    return data;
  },

  async signup(data: SignupRequest): Promise<AuthResponse> {
    const response = await fetch(`${BASE_URL}/v1/account/auth/signup`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (resData && resData.userProfileResponse && !resData.user) {
      resData.user = resData.userProfileResponse;
    }
    return resData;
  },

  async getFiles(projectId: string): Promise<FileNode[]> {
    const response = await fetch(`${BASE_URL}/v1/workspace/projects/${projectId}/files`, {
  ...
  async getFileContent(projectId: string, path: string): Promise<string> {
    const response = await fetch(
      `${BASE_URL}/v1/workspace/projects/${projectId}/files/content?path=${path}`,
      {
        headers: { ...getAuthHeaders() },
      }
    );
  ...
  async deploy(projectId: string): Promise<DeployResponse> {
    const response = await fetch(`${BASE_URL}/v1/workspace/projects/${projectId}/deploy`, {
      method: "POST",
      headers: { ...getAuthHeaders() },
    });
  ...
  async getProjects(): Promise<ProjectSummaryResponse[]> {
    const response = await fetch(`${BASE_URL}/v1/workspace/projects`, {
      headers: { ...getAuthHeaders() },
    });
  ...
  async getProject(id: string): Promise<ProjectResponse> {
    const response = await fetch(`${BASE_URL}/v1/workspace/projects/${id}`, {
      headers: { ...getAuthHeaders() },
    });
  ...
  async downloadProjectZip(id: string): Promise<Blob> {
    const response = await fetch(`${BASE_URL}/v1/workspace/projects/${id}/files/download-zip`, {
      headers: { ...getAuthHeaders() },
    });
  ...
  async getProjectMembers(projectId: string): Promise<ProjectMember[]> {
    const response = await fetch(`${BASE_URL}/v1/workspace/projects/${projectId}/members`, {
      headers: { ...getAuthHeaders() },
    });
  ...
  async updateMemberRole(projectId: string, userId: number, role: ProjectRole): Promise<void> {
    const response = await fetch(`${BASE_URL}/v1/workspace/projects/${projectId}/members/${userId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", ...getAuthHeaders() },
  ...
  async getChatHistory(projectId: string): Promise<ChatMessage[]> {
    const response = await fetch(`${BASE_URL}/v1/intelligence/chat/projects/${projectId}`, {
      headers: { ...getAuthHeaders() },
    });
  ...
  async streamChat(
    ...
    fetch(`${BASE_URL}/v1/intelligence/chat/stream`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...getAuthHeaders() },
  ...
  async getSubscription(): Promise<{
    ...
    const response = await fetch(`${BASE_URL}/v1/account/api/me/subscription`, {
      headers: { ...getAuthHeaders() },
    });
    if (!response.ok) {
  ...
  async createCheckoutSession(planId: number): Promise<{ checkoutUrl: string }> {
    const response = await fetch(`${BASE_URL}/v1/account/api/payment/checkout`, {
      method: "POST",
      headers: {
  ...
  async openCustomerPortal(): Promise<{ portalUrl: string }> {
    const response = await fetch(`${BASE_URL}/v1/account/api/payment/portal`, {
      method: "POST",
      headers: {
        ...getAuthHeaders(),
      },
